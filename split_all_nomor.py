...
def pecah_database_nomor(source_db, prefix_target, jumlah_pecahan=2):
    ...
    for table in tables:
        ...
        for i in range(jumlah_pecahan):
            ...
            # Nama file hasil pecahan: misal nomor11.db, nomor12.db, dst.
...
# Loop otomatis dari nomor1.db sampai nomor7.db
for i in range(1, 8):
    src = f"db/nomor{i}.db"
    if os.path.exists(src):
        print(f"\nMulai memecah {src}...")
        # Masing-masing file dipecah jadi 2 bagian (ubah angkanya jika butuh lebih banyak pecahan)
        pecah_database_nomor(src, f"nomor{i}", jumlah_pecahan=2)

print("\nSemua proses pemecahan database Nomor selesai!")
